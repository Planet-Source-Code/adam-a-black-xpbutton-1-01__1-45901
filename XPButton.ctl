...
Private Sub hTimer_Timer()
    ...
    MouseIn = WindowFromPoint(mPos.x, mPos.y) = UserControl.hwnd
    ...
    'mouse or space are down
    ElseIf (MouseDown = True) Or (SpaceDown = True) Then
        'the usercontrol has foward focus
        If UserControl.Parent.hwnd = GetForegroundWindow Then
            
            If MouseDown = True Then
                'the left mouse is no longer down (from interruption)
                If GetAsyncKeyState(vbKeyLButton) = 0 Then
                    'get state of shift key
                    bshift = IIf(Not GetAsyncKeyState(vbKeyShift) = 0, 1, 0)
                    'get cursor position relative to usercontrol.
                    Call ScreenToClient(UserControl.hwnd, mPos)
                    'Raise MouseUp event.
                    Call UserControl_MouseUp(vbKeyLButton, bshift, CSng(mPos.x), CSng(mPos.y))
                End If
            'SpaceDown <MUST> = True
            Else
                'the space button is no longer down
                If GetAsyncKeyState(vbKeySpace) = 0 Then
                    'get state of shift key
                    bshift = IIf(Not GetAsyncKeyState(vbKeyShift) = 0, 1, 0)
                    'raise KeyUp event
                    Call UserControl_KeyUp(vbKeySpace, CInt(bshift))
                End If
            End If
        
        'parent form has lost focus while mouse/space bar is down (example, Alt+Tab)
        Else
            'the mouse is within the usercontrol
            If MouseIn Then
                'draw mousein
...
Private Sub UserControl_ExitFocus()
    If (Extender.Default = True And Ambient.DisplayAsDefault = False) Or (Extender.Default = False) Then
        ...
        If (MouseDown Or SpaceDown) = True Then
            MouseDown = False
            SpaceDown = False
            ReleaseCapture
        End If
...
Private Sub UserControl_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    If SpaceDown = True And KeyCode = vbKeySpace Then
        ...
        Call GetCursorPos(mPos)
        'the cursor isn't over the usercontrol
        If Not (WindowFromPoint(mPos.x, mPos.y) = UserControl.hwnd) Then
            hTimer.Enabled = False
            m_ButtonState = DrawButton(2)
            MouseOver = False
...
Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    If Button = vbLeftButton And Not NoMD Then
        m_ButtonState = DrawButton(4)
        MouseDown = True
        hTimer.Enabled = True
    ElseIf Button = vbRightButton And MouseDown = False Then
        ReleaseCapture
    End If
End Sub
...
Private Sub DrawTextTohWnd(htext As String)
    ...
    vh = DrawText(UserControl.hdc, htext, -1, hRect, DT_CALCRECT Or DT_CENTER Or DT_WORDBREAK)
    ...
    'now draw the text
    DrawText UserControl.hdc, htext, -1, hRect, DT_WORDBREAK Or DT_CENTER
End Sub
...
Private Sub CreateRoundRegion()
    Dim lRet    As Long
    Dim l       As Long
    
    'Create Form with Rounded Corners
    lRet = CreateRoundRectRgn(0, 0, UserControl.ScaleWidth + 1, UserControl.ScaleHeight + 1, 2, 2)
    l = SetWindowRgn(UserControl.hwnd, lRet, True)
    
    DeleteObject lRet
    UserControl.Refresh
End Sub

Private Function DrawButton(lstate As Byte) As Byte
